...
public class MintUserHoldServiceImpl extends ServiceImpl<MintUserHoldMapper, MintUserHold> implements MintUserHoldService {
    ...
    public MintUserHold getMintUserHold(String userAddress, Long deployId) {
        List<MintUserHold> mintUserHoldList = lambdaQuery().eq(MintUserHold::getUserAddress, userAddress).eq(MintUserHold::getDeployId, deployId).list();
        MintUserHold one = null;
        if (CollectionUtils.isEmpty(mintUserHoldList)) {
            ...
            save(one);
            return lambdaQuery().eq(MintUserHold::getUserAddress, userAddress).eq(MintUserHold::getDeployId, deployId).one();
        } else if (mintUserHoldList.size() > 1){
            baseMapper.removeHold();
            return lambdaQuery().eq(MintUserHold::getUserAddress, userAddress).eq(MintUserHold::getDeployId, deployId).one();
        } else {
            one = mintUserHoldList.get(0);
        }
        return one;
    }
    @Override
