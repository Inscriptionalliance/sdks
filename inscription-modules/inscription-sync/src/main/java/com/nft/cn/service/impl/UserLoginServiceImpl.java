...
public class UserLoginServiceImpl extends ServiceImpl<UserLoginMapper, UserLogin> implements UserLoginService {
    ...
    public void login(UUser user) {
        UserLogin one = lambdaQuery().eq(UserLogin::getUserAddress, user.getUserAddress()).ge(UserLogin::getCreateTime, LocalDate.now()).one();
        if (one != null) {
    ...
    @Override
    public Integer countByUserAddress(LocalDate startDay, LocalDate endDay) {
        return baseMapper.countByUserAddress(startDay, endDay);
    }
}
