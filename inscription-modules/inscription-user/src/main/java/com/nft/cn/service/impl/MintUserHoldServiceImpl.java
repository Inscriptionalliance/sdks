...
public class MintUserHoldServiceImpl extends ServiceImpl<MintUserHoldMapper, MintUserHold> implements MintUserHoldService {
    ...
    public MintUserHold getMintUserHold(String userAddress, Long deployId) {
        ...
        if (one == null) {
            while (true) {
                String key = "mint-MintUserHoldSave" + userAddress;
                boolean flag = redisUtils.setNx(key, "mint-MintUserHoldSave" + userAddress, 5);
                if (flag) {
                ...
                one = lambdaQuery().eq(MintUserHold::getUserAddress, userAddress).eq(MintUserHold::getDeployId, deployId).last("limit 1").one();
                if (one != null) {
                    return one;
                }
            }
